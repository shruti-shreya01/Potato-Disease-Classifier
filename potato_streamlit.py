...
# # Path to the pickle file
# file_path = r"C:\Users\shrey\Downloads\potato_pickle_final.pkl"
...
# Constants
IMAGE_SIZE = 256
FILE_PATH = "potato_pickle_final (1).pkl"  # Path to your pickle file
...
def load_model():
    with open(FILE_PATH, "rb") as f:
        data = pickle.load(f)
    model = tf.keras.models.model_from_json(data["architecture"])
    model.set_weights(data["weights"])  # Set the model weights
    return model
...
if uploaded_file is not None:
    ...
    # Log input shape
    st.write(f"Input shape: {img_array.shape}")  # Check the input shape

    # Make prediction
    prediction = model.predict(img_array)
    
    # Log raw prediction
    st.write(f"Raw Prediction: {prediction}")  # Check the raw output
    ...
    # Log predicted class and confidence
    st.write(f"Predicted Class: {predicted_class}, Confidence: {confidence:.2f}")  # Log results
